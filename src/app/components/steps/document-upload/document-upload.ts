...
import { MIN_BANK_STATEMENTS, MIN_TAX_RETURNS } from '../../../constants/wizard-config'
...
export class DocumentUpload implements OnInit, OnDestroy {
  ...
  private loadSavedData(): void {
    ...
    if (formData.documents) {
      ...
      const hasValidFiles = (files: File[] | undefined): files is File[] =>
  ...
  removeFile(fileType: string, index: number): void {
    switch (fileType) {
      case 'governmentId':
        ...
        break
      case 'businessRegistration':
        this.businessRegistrationFiles = this.businessRegistrationFiles.filter(
          (_, i) => i !== index
        )
        break
      case 'bankStatements':
    ...
    this.documentForm.patchValue(
      {
        ...
        businessRegistration:
          this.businessRegistrationFiles.length > 0 ? this.businessRegistrationFiles : null,
        bankStatements:
          this.bankStatementFiles.length >= MIN_BANK_STATEMENTS ? this.bankStatementFiles : null,
        taxReturns: this.taxReturnFiles.length >= MIN_TAX_RETURNS ? this.taxReturnFiles : null,
  ...
  isFormValid(): boolean {
    return (
      this.governmentIdFiles.length > 0 &&
      this.businessRegistrationFiles.length > 0 &&
      this.bankStatementFiles.length >= MIN_BANK_STATEMENTS &&
      this.taxReturnFiles.length >= MIN_TAX_RETURNS
    )
  }
