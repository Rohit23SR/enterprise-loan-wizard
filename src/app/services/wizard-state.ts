...
import {
  ...
  STORAGE_EXPIRATION_DAYS,
  WIZARD_STORAGE_KEY,
  SUBMITTED_APP_STORAGE_KEY,
} from '../constants/wizard-config'
...
export class WizardState {
  ...
  private loadFromStorage(): void {
    try {
      ...
      if (stored) {
        ...
        if (daysDiff < STORAGE_EXPIRATION_DAYS) {
  ...
  async submitApplication(): Promise<{ success: boolean; referenceNumber?: string }> {
    try {
      ...
      localStorage.setItem(SUBMITTED_APP_STORAGE_KEY, JSON.stringify(submittedData))
  ...
  private validateReview(data: Partial<ReviewConfirmation>): boolean {
    return !!(data.certifyAccurate && data.agreeToTerms && data.consentToCreditCheck)
  }
}
